...
Utilizarei *mock* como um termo geral para facilitar no entendimento. Mesmo que existam outras nomenclaturas (como *fake* para dados, *spies* para verificação de funções e *stubs* para sobrescrever comportamento), é bem comum que chamemos tudo de *mock* no dia-a-dia.
...
Se quiser fazer um exercício, crie um novo caso de teste onde o `mock` da função `isAlive` retornar `false`. :)
...
Vale apontar que, utilizando `jest.mock` você não vai conseguir passar uma variável para a função (como segundo parâmetro) a menos que ela tenha `mock` como prefixo no nome da variável. Ou seja:

```js
const mockIsAlive = {
  isAlive: jest.fn();
};
const isAlive = mockIsAlive;

// This works
jest.mock('./is-alive', () => mockIsAlive);

// This doesn't work
jest.mock('./is-alive', () => isAlive);
```
