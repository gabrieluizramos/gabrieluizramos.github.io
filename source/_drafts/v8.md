...
Uma **fila** se assemelha às filas que encontramos em super-mercados, bancos ou shows, onde você é atendido por ordem de chegada e não é correto alguma pessoa ser atendia antes de qualquer outra que já estava ali anteriormente (tirando os casos onde existem filas preferenciais).
...
**JavsScript** é uma linguagem conhecida também por tratar funções como qualquer outro valor. Você pode passar funções como parâmetro, retorná-las de outras funções e atribuí-las à variáveis normalmente. Essa caracterísica é conhecida como [**função First-class**](https://developer.mozilla.org/pt-BR/docs/Glossario/Funcao-First-class) (ou "**fist-class functions**").
...
Você verá encontrará semelhante à isso:
...
#### C++ APIs
São as as **APIs** providas para o **NodeJS**, já que é escrito em C++. Algumas funções existentes nas APIs web foram reescritas (como o `setTimeout`) e podem ser utilizadas nesse contexto também, mas muitas outras são específicas: como o módulo `fs` responsável por acessar o sistema de arquivos da máquina e realizar operações com dados do disco rígido.
